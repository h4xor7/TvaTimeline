...
var MemStorage = class {
  ...
  async getAchievementsBySession(sessionId) {
...
async function registerRoutes(app2) {
  ...
  app2.get("/api/achievements/:sessionId", async (req, res) => {
    try {
      const { sessionId } = req.params;
      const achievements2 = await storage.getAchievementsBySession(sessionId);
      res.json(achievements2);
    } catch (error) {
      res.status(500).json({ message: "Failed to fetch achievements" });
    }
  });
  app2.get("/api/github/user/:username", async (req, res) => {
    try {
  ...
  app2.get("/api/github/repos/:username", async (req, res) => {
    try {
      const { username } = req.params;
